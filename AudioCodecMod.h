...
static inline void AudioCodec_init(void) {
  ...
  Wire.write((uint8_t)0x02); // turn everything on except mic
  ...
  // setup ADCs
  #if ADCS == 0
    DIDR0 = (1 << ADC3D); // turn off digital inputs for ADC3
  #elif (ADCS == 1) || (ADCS == 2)
    ADMUX = 0x43; // start with ADC3 - internal VCC for Vref
...
//Function to read from SRAM, "&" is required infront on MSB and LSB
//Receives pointers to MSB and LSB and returns int16 value stored in memory
int16_t readSRAM(uint8_t msb, uint8_t lsb){
  //READ-------------------------------------------------
  //Set data port to input
  DDRD &= ~0xDF; //Set PORTD to input excluding PIN5
  DDRB &= ~1; //set B0 to input
  __asm__("nop\n\t");
  
  //MSB
  //set read enable pin low
  PORTC &= ~7;
  __asm__("nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t"); 
  //read MSB (PORTD w/ PIN5 swaped with PIN0 of PORTB)
  msb = PIND;
  msb ^= (-((PINB>>0)&1)^msb)&(1<<5);
  __asm__("nop\n\t" "nop\n\t");
  //set read enable pin high
  PORTC |= 7;
  __asm__("nop\n\t" "nop\n\t");
  //LSB address toggle (B1)
  PORTB ^= 2;
...
//Function to write to SRAM, "&" is required infront on MSB and LSB
//Receives pointers to MSB, LSB and sample and writes int16 value to memory
int16_t writeSRAM(uint8_t msb, uint8_t lsb, int16_t value){
  //WRITE------------------------------------------------
  //set data port to output
  DDRD |= 0xDF; //Set PORTD to output excluding PIN5
  DDRB |= 1; //set B0 to output
  __asm__("nop\n\t");
  
  //MSB
  msb = value>>8;
  //write to port
  PORTD = msb&~(1<<5); //set PORTD to MSB except PIN5
  PORTB ^= (-((msb>>5)&1)^PORTB)&(1<<0); //set PIN0 in PORTB to  BIT5 of MSB
  __asm__("nop\n\t" "nop\n\t");
  //set write enable pin low
  PORTC = (1|PORTC)&~6;
  __asm__("nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t");
  //set write enable pin high
  PORTC |= 7;
  __asm__("nop\n\t" "nop\n\t" "nop\n\t");
  //LSB address toggle (B1)
  PORTB ^= 2;
  ...
  lsb = value&0xFF;
  //write to port
  PORTD = lsb&~(1<<5); //set PORTD to LSB except PIN5
  PORTB ^= (-((lsb>>5)&1)^PORTB)&(1<<0); //set PIN0 in PORTB to  BIT5 of LSB
  __asm__("nop\n\t" "nop\n\t" "nop\n\t");
  //set write enable pin low
  PORTC = (1|PORTC)&~6;
  __asm__("nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t"); 
